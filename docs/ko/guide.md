...
- **네이티브 IME** — Windows 입력기 프레임워크에 직접 통합되어, 기본 한글 입력기와 동등한 수준의 호환성을 목표로 합니다
- **Colemak 영문 + 두벌식 한글** — 한/영 전환: `한글 키` 또는 `우측 Alt`
- **Colemak / QWERTY 즉시 전환** — `Win+Space`로 전환 (설정에서 변경 가능), 다른 사용자와 컴퓨터를 공유할 때 편리합니다
- **Win+key Colemak 리맵** — Colemak 모드에서 `Win+E`(탐색기), `Win+R`(실행) 등 Windows 셸 단축키도 Colemak 배열을 따릅니다 (설정에서 끌 수 있음)
...
- **시스템 트레이 설정** — 트레이 아이콘 우클릭으로 설정 변경 (CapsLock 동작, ㅔ 키 위치, Win+key 리맵, 전환 단축키)
- **설정 자동 저장** — CapsLock, ㅔ 키 위치, Win+key 리맵, 단축키 설정이 레지스트리에 저장되어 재시작 후에도 유지 (Colemak 모드는 항상 기본으로 시작)
...
**Windows 설정 > 앱 > 설치된 앱**에서 "Kolemak IME"를 제거합니다. 또는 `kolemak-install.exe`를 다시 실행하면 제거 여부를 묻는 대화 상자가 표시됩니다.
...
<kbd>Win</kbd>+<kbd>Space</kbd>를 눌러 Colemak과 QWERTY 레이아웃을 전환합니다. 전환 단축키는 설정에서 변경할 수 있습니다.

### Win+key Colemak 리맵

Colemak 모드에서는 Windows 셸 단축키도 Colemak 배열을 따릅니다. 예를 들어, 물리적 `K` 키 위치에서 `Win+K`를 누르면 Colemak에서 `K`→`E` 이므로 `Win+E`(파일 탐색기)가 실행됩니다.

QWERTY 모드에서는 리맵이 자동으로 비활성화되어 일반 QWERTY 배열의 단축키가 동작합니다.

> 설정에서 "Win+키 Colemak 리맵" 체크박스를 해제하면 이 기능을 끌 수 있습니다.
...
| Win+키 Colemak 리맵 | Colemak 모드에서 Win+alpha 단축키를 Colemak 배열로 리맵 |
| Colemak/QWERTY 전환 단축키 | 기본: `Win+Space` |
...
| 설정 | 영문 `;` 위치 | 한글 `ㅔ` 위치 |
|------|-------------|--------------|
| **켬** (Changed) | Colemak `;`이 `P` 키에 위치 | `ㅔ`가 `;` 키로 이동 |
| **끔** (Unchanged) | Colemak `;`이 `P` 키에 위치 | `ㅔ`가 기존 `P` 키에 유지 |

- **켬** — `ㅔ`를 물리적 `;` 키 위치로 이동합니다. 영문 `;`와 한글 `;`가 같은 키에 있어 통일된 레이아웃을 원할 때.
- **끔** — `ㅔ`를 원래의 물리적 `P` 키에 유지합니다. 한글 입력을 기존 QWERTY 두벌식과 동일하게 사용하려면.
...
### 업데이트 후 설정 다이얼로그가 열리지 않을 때

Kolemak을 업데이트(재설치)한 후, 트레이 아이콘의 설정 다이얼로그가 열리지 않거나 입력이 비정상적으로 동작할 수 있습니다. 이는 시스템이 이전 버전의 DLL을 메모리에 계속 유지하고 있기 때문입니다.

**해결 방법:**

1. **설정 > 시간 및 언어 > 언어 및 지역 > 한국어 > 언어 옵션 > 키보드**에서:
   - Microsoft 입력기를 **추가**
   - Kolemak IME를 **제거**
   - Kolemak IME를 다시 **추가**
   - Microsoft 입력기를 **제거**

이 과정을 통해 시스템이 새 DLL을 다시 로드합니다.
